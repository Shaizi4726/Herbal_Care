$('#order-track').on('click', function(event) {
  let id = $('#order-id-input').val().toUpperCase();
  console.log(id);

  /* AJAX request for order tracking data */
  $.ajax({
    type: 'get',
    url: '/track/order',
    data: {id: id},
    success: function(response) {
      $('#track-order-details').removeClass('collapse');
      $('#success').removeClass('collapse');
      $('#fail').addClass('collapse');

      let shipping = response['shipping'];
      let status = shipping['status'];
      
      $('#order-no').html('#' + response['order_no']);
