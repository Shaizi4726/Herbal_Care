...
<!--  <div class="card-body">
    <form action="{{route('order.update',$order->id)}}" method="POST">
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          <option value="new" {{($order->payment_status=='paid' ) ? 'disabled' : ''}}  {{(($order->payment_status=='unpaid')? 'selected' : '')}}>unpaid</option>
          <option value="process" {{($order->payment_status=='paid') ? 'disabled' : ''}}  {{(($order->status=='paid')? 'selected' : '')}}>paid</option>
