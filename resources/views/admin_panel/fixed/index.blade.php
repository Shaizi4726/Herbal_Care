...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
      ...
      <a href="{{route('fixed.create')}}" class="btn btn-primary btn-sm float-right" data-toggle="tooltip" data-placement="bottom" title="Add User"><i class="fas fa-plus"></i> Add Fixed Banner</a>
    </div>
    <div class="card-body">
          ...
          <tbody>
            @foreach($banner_fixeds as $banner_fixed)   
                <tr>
                    ...
                    <td>
                        <a href="{{route('fixed.edit',$banner_fixed->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{route('fixed.destroy',[$banner_fixed->id])}}">
                          @csrf 
                          @method('delete')
                              <button class="btn btn-danger btn-sm dltBtn" data-id={{$banner_fixed->id}} style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" data-placement="bottom" title="Delete"><i class="fas fa-trash-alt"></i></button>
                        </form>
                    </td>
                    {{-- Delete Modal --}}
                        ...
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              <form method="post" action="{{ route('fixed.destroy',$user->id) }}">
                                @csrf 
                                @method('delete')
